...
document.addEventListener('DOMContentLoaded', () => {
    ...
    async function loadConfig() {
        try {
            const response = await fetch('http://api.guajindou.xyz/api/config');
    ...
    if (myTicketsBtn) {
        myTicketsBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch(`http://api.guajindou.xyz/api/user-info?address=${currentAccount}`);
    ...
    async function handlePurchase(ticketType, quantity = 1) {
        ...
        try {
            ...
            if (receipt.status === 1) {
                ...
                try {
                    const verifyRes = await fetch('http://api.guajindou.xyz/api/verify-payment', {
                        method: 'POST',
    ...
    if (startScratchBtn) {
        startScratchBtn.addEventListener('click', async () => {
            // Call API to Play
            try {
                const res = await fetch('http://api.guajindou.xyz/api/play', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({
                        address: currentAccount,
    ...
    if (claimRewardBtn) {
        claimRewardBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('http://api.guajindou.xyz/api/claim', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
    ...
    if (claimPityBtn) {
        claimPityBtn.addEventListener('click', async () => {
            ...
            try {
                const res = await fetch('http://api.guajindou.xyz/api/claim-pity', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ address: currentAccount })
                });
    ...
    if (shippingForm) {
        shippingForm.addEventListener('submit', async (e) => {
            ...
            try {
                const res = await fetch('http://api.guajindou.xyz/api/submit-address', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ 
                        address: currentAccount,
